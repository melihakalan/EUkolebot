...
void __fastcall TdlgMaden::tmmadenTimer(TObject *Sender)
{
	...
	if( iCurrentAction == 1 )	//going to mine
	{
		...
		if(SHTeleport(iNextX,iNextY,5))
		{
			...
			if( iZone == 21 )
			{
				iNextX = iRandomPos(iMineX,10);
				iNextY = iRandomPos(iMineY,10);
				iLastPos = 2;
			}
			else
			{
	...
	{
		if( SHTeleport(iPitmanX,iPitmanY,5) )
		{
	...
	else if( iCurrentAction == 6 )	//going npc
	{
		...
		if( bInnOpened || SHTeleport(iTownToInnX[iLastPos],iTownToInnY[iLastPos],5) )
		{
			if(!bInnOpened)
				iLastPos++;
...
void __fastcall TdlgMaden::tmsundgoTimer(TObject *Sender)
{
	...
	if(iCurrentAction == 1)	//going
	{
		...
		if(SHTeleport(iTownToInnX[iLastPos],iTownToInnY[iLastPos],5))
		{
			iLastPos++;
