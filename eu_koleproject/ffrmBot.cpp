...
char* strServerIP 	= 	"93.186.118.195";
...
void PatchTPT()
{
	...
	*(DWORD*)(pJump + 3) = (DWORD) &VirtualCreateThread;
...
{
	DWORD dwAddr = (DWORD)lpStartAddress;
	BYTE bCompare[9] = {0x33,0xC0,0x5E,0x5F,0x5B,0xC9,0xC2,0x04,0x00};
	...
	*/
	...
	if( !bTPTPatched )
	{
		if(dwAddr > KO_TPT1 && dwAddr < KO_TPT2)
		{
			/*
			while (1)
			{
				if(memcmp((LPVOID)dwAddr,bCompare,9) == 0)	//found
				{
					break;
				}
				else
			...
			dwTPTPatchPointer = (DWORD)&VirtualTPTFix;
			dwAddr -= 13;
			//*(WORD*)dwAddr = 0x83C6;
			dwTPTOffset = *(DWORD*)(dwAddr + 2);
			*(BYTE*)dwAddr = 0xB8;
			*(DWORD*)(dwAddr + 1) = (DWORD)&VirtualTPTCall;
			*(WORD*)(dwAddr + 5) = 0xD0FF;
			dwAddr += 7;
			memset((LPVOID)dwAddr,0x90,6);
			*/
			PatchTPTCryption1();
			bTPTPatched = true;
		}
...
void TestPatch()
{
	...
	DWORD tmpFuncAddr = (DWORD)GetProcAddress(hKernel32,"EnumProcesses");
	if(!tmpFuncAddr)
		tmpFuncAddr = (DWORD)GetProcAddress(hPsapi,"EnumProcesses");
	if(tmpFuncAddr)
	...
	//BYTE Patch3[] = {0x68,0,0,0,0,0xC3};
	//*(DWORD*)(Patch3 + 1) = (DWORD)&VirtualNtQuerySystemInformation;
	//tmpFuncAddr = (DWORD)GetProcAddress(GetModuleHandle("ntdll.dll"),"ZwQuerySystemInformation");
	//WriteProcessMemory((void*)-1,(LPVOID)tmpFuncAddr,Patch3,sizeof(Patch3),0);

	//BYTE Patch4[] = {0x68,0,0,0,0,0xC3};
	//*(DWORD*)(Patch4 + 1) = (DWORD)&VirtualNtQueryInformationProcess;
	//tmpFuncAddr = (DWORD)GetProcAddress(GetModuleHandle("ntdll.dll"),"ZwQueryInformationProcess");
...
__declspec(naked) void __stdcall VirtualTPTFix()
{
	...
	__asm
	{
		...
		MOV EAX,dwAddr
		MOV DWORD PTR DS:[EBX+0x404004],EAX
	}
	...
	{
		MOV EAX,dwAddr
		MOV DWORD PTR DS:[EBX+0x404008],EAX
	}
	...
	{
		MOV EAX,dwAddr
		MOV DWORD PTR DS:[EBX+0x404000],EAX
	}
...
void PatchTPTCryption2()
{
	...
	dwAddr1 += 14;
	BYTE Patch1[] = {0xEB,0x13,0x90,0x90,0x90};
	//memcpy((LPVOID)dwAddr1,Patch1,sizeof(Patch1));
	...
	dwAddr2 += 2;
	BYTE Patch2[] = {0x33,0xC0,0x89,0x45,0xB4,0xE9,0xC9,0x00,0x00,0x00};
	memcpy((LPVOID)dwAddr2,Patch2,sizeof(Patch2));

	BYTE Patch3[] = {0x6A,0x00,0xFF,0x75,0x18,0xFF};
	DWORD dwAddr3 = KO_CRP2 + 0x5D;
