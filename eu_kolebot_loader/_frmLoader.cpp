...
void HandleProcess(){
	...
	*(pFileName + 1) = '\0';
	...
	CopyFile("kolebot.dll",strPath,false);
...
void LoadProcess(){
	...
	*(pFileName + 1) = '\0';
	...
	char strParam[8]; memset(strParam,0,8);
	*strParam = 0x20;
	itoa( GetCurrentProcessId(), (char*)(strParam + 1), 10);

	char strMainParam[256]; memset(strMainParam,0,256);
	char pQuota[2]; *pQuota = 0x22; *(pQuota + 1) = 0x00;
	*strMainParam = 0x22;
	strcat(strMainParam,frmLoader->txtPath->Text.c_str());
	strcat(strMainParam,pQuota);
	strcat(strMainParam,strParam);

	bool bProcess = CreateProcess(NULL,strMainParam,NULL,NULL,true,NULL,NULL,strPathDir,&si,&pi);
	...
	*(pFileName + 1) = '\0';
	strcat(strPath,"initloader.kole");
	CreateFileA(strPath,GENERIC_READ,FILE_SHARE_READ,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
	WriteProcessMemory(pi.hProcess,lpVirtualMem,"kolebot.dll",iMemSize - 1,NULL);
...
void CheckDLL()
{
	...
	hFile = CreateFileA("kolebot.dll",FILE_GENERIC_READ,FILE_SHARE_READ,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
	if(hFile == INVALID_HANDLE_VALUE)
	{
		...
		{
			MessageBox(frmLoader->Handle,"kolebot.dll bulunamadi!","Hata!",MB_ICONSTOP);
			Application->Terminate();
			return;
		}
	}
	...
	{
		MessageBox(frmLoader->Handle,"kolebot.dll bozuk! yeniden indiriniz!","Hata!",MB_ICONSTOP);
		Application->Terminate();
	}
}
